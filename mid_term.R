...
hdr.data <-  parLapply(cluster2,1:length(img.names),function(x,y=si.hdrdata,z=img.names){return ((rbind(y[[x]][,c("name","value")],c("image_id",z[x]))))})
...
clusterExport(cluster3,c("col.list","hdr.data"))
clusterEvalQ(cluster3,c(library("dplyr")))
hdr.filter.data <-  parLapply(cluster3,1:length(hdr.data),function(x,y=hdr.data,z=col.list){return(filter(y[[x]],y[[x]][,1] %in% z))})
stopCluster(cluster3)

cluster4 <- makeCluster(no_cores,type = "PSOCK")
clusterExport(cluster4,c("hdr.filter.data"))
hdr.values<-parLapply(cluster4,1:length(hdr.filter.data),function(x,y=hdr.filter.data){y[[x]][,2]})
